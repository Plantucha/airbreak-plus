...
class AirMiniSppTransport:
    ...
    def _decode_json_payload(self, vcid: int, payload: bytes) -> dict:
        ...
        try:
            ...
            if len(payload) >= 2:
                ...
                candidate = payload[2:2 + size] if size == len(payload) - 2 else b''
                try:
                    message = json.loads(candidate.decode("utf-8"))
                except (UnicodeDecodeError, json.JSONDecodeError):
                    pass
                else:
                    if isinstance(message, dict):
                        return message
            raise TransportError(f"non-JSON object on FIG VCID 0x{vcid:04x}") from first_error
        if not isinstance(message, dict):
            raise TransportError(f"non-JSON object on FIG VCID 0x{vcid:04x}")
        return message
    ...
    def _send_rpc(self, method: str, params: object | None = None, *,
        ...
        try:
            ...
            with self._response_condition:
                self._pending_ids.discard(rpc_id)
