...
// Register/value pairs, with an unused register number marking delays in ms.
#define ILI_INIT_DELAY 0xFFFF
static const uint16 ili9325_init[][2] = {
    {0x01, 0x0000}, // SS=0 (flip horizontal scan for 180 degrees)
    {0x02, 0x0700},
    {0x03, 0x0030}, // BGR=0 (emWin RGB565), ID1=1, ID0=1
    {0x04, 0x0000},
    {0x08, 0x0207},
    {0x09, 0x0000},
    {0x0A, 0x0000},
    ...
    {0x60, 0x2700}, // GS=0 (flip vertical scan for 180 degrees)
    {0x61, 0x0001},
    {0x6A, 0x0000},
...
{
    unsigned int step;
    for (step = 0; step < sizeof(ili9325_init) / sizeof(ili9325_init[0]); step++) {
        uint16 reg = ili9325_init[step][0];
        uint16 value = ili9325_init[step][1];
        if (reg == ILI_INIT_DELAY)
            lcd_delay_ms(value);
        else
            ili_write_reg(reg, value);
    }
